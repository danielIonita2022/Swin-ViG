...
def get_sliding_window_generator(image_size: Tuple[int, ...], tile_size: Tuple[int, ...], tile_step_size: float,
    ...
    if len(tile_size) < len(image_size):
        print("Entered len(tile_size) < len(image_size)")
...
def predict_sliding_window_return_logits(network: nn.Module,
    ...
    print("Before network.to(device)" + str(device) + "network.eval()")
    network = network.to(device)
    network.eval()
    print("After network.to(device)" + str(device) + "network.eval()")

    print("Before empty_cache(device)")
    empty_cache(device)
    print("After empty_cache(device)")
    ...
    with torch.no_grad():
        ...
        with torch.autocast(device.type, enabled=True) if device.type == 'cuda' else dummy_context():
            ...
            print("Before computing gaussian")
            if use_gaussian:
            ...
            print("After computing gaussian")

            print("Before get_sliding_window_generator")
            slicers = get_sliding_window_generator(data.shape[1:], tile_size, tile_step_size, verbose=verbose)
            print("After get_sliding_window_generator")
