...
if __name__ == "__main__":
    source = '/mnt/hdd1/home/danielionita/data/RAVIR/'  # Update this path to your dataset location

    dataset_name = 'Dataset501_RAVIR'
    ...
    with multiprocessing.get_context("spawn").Pool(8) as p:
        # Training set
        valid_ids = subfiles(join(train_source, 'training_images'), join=False, suffix='png')
        ...
        for v in valid_ids:
            r.append(
                p.starmap_async(
                    ...
                    ((
                         join(train_source, 'training_images', v),
                         join(train_source, 'training_masks', v),
        ...
        # Test set
        valid_ids = subfiles(join(test_source, 'testing_images'), join=False, suffix='png')
        for v in valid_ids:
            r.append(
                p.starmap_async(
                    load_and_convert_case,
                    ((
                         join(test_source, 'testing_images', v),
                         join(test_source, 'testing_masks', v),
    ...
    generate_dataset_json(join(nnUNet_raw, dataset_name), {0: 'IR'}, {'background': 0, 'vessel': 1},
