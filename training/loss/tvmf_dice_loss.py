...
class Adaptive_tvMF_DiceLoss(nn.Module):
    ...
    def _tvmf_dice_loss(self, score, target, kappa):
        ...
        score = F.normalize(score, p=2, dim=[1,2,3])
        target = F.normalize(target, p=2, dim=[1,2,3])
        cosine = torch.sum(score * target, dim=[1,2,3])
        cosine_mean = torch.mean(cosine)
        intersect =  (1. + cosine_mean).div(1. + (1.- cosine_mean).mul(kappa)) - 1.
        loss = (1 - intersect)**2.0

        #print("Cosine Similarity:", cosine_mean)
